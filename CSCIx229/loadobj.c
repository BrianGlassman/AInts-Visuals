...
static char* readline(FILE* f)
{
   ...
   while ((ch = fgetc(f)) != EOF)
   {
      ...
      if (k>=linelen)
      {
         ...
         if (!line) Fatal(10, "Out of memory in readline\n");
      }
      //  End of Line
      if (CRLF(ch))
      {
...
static void readfloat(char* line,int n,float x[])
{
   for (int i=0;i<n;i++)
   {
      ...
      if (!str)  Fatal(11, "Premature EOL reading %d floats\n",n);
      if (sscanf(str,"%f",x+i)!=1) Fatal(12, "Error reading float %d\n",i);
   }
}
...
static void readcoord(char* line,int n,float* x[],int* N,int* M)
{
   ...
   if (*N+n > *M)
   {
      ...
      *x = (float*)realloc(*x,(*M)*sizeof(float));
      if (!*x) Fatal(13, "Cannot allocate memory\n");
   }
   //  Read n coordinates
...
static void LoadMaterial(const char* dir, const char* _file)
{
   ...
   while ((line = readline(f)))
   {
      ...
      if ((str = readstr(line,"newmtl")))
      {
         ...
         if (!mtl[k].name) Fatal(14, "Cannot allocate %d for name\n",l+1);
...
int LoadOBJ(const char* dir, const char* _file)
{
   ...
   //  Open file
   FILE* f = fopen(file,"r");
   if (!f) Fatal(15, "Cannot open file %s\n",file);
   ...
   while ((line = readline(f)))
   {
      ...
      else if (line[0]=='f')
      {
         ...
         while ((str = getword(&line)))
         {
            ...
            {
               if (Kv<0 || Kv>Nv/3) Fatal(16, "Vertex %d out of range 1-%d\n",Kv,Nv/3);
               if (Kn<0 || Kn>Nn/3) Fatal(17, "Normal %d out of range 1-%d\n",Kn,Nn/3);
               if (Kt<0 || Kt>Nt/2) Fatal(18, "Texture %d out of range 1-%d\n",Kt,Nt/2);
            }
            //  Try Vertex//Normal pairs
            else if (sscanf(str,"%d//%d",&Kv,&Kn)==2)
            {
               if (Kv<0 || Kv>Nv/3) Fatal(19, "Vertex %d out of range 1-%d\n",Kv,Nv/3);
               if (Kn<0 || Kn>Nn/3) Fatal(20, "Normal %d out of range 1-%d\n",Kn,Nn/3);
               Kt = 0;
            }
            //  Try Vertex index
            else if (sscanf(str,"%d",&Kv)==1)
            {
               if (Kv<0 || Kv>Nv/3) Fatal(21, "Vertex %d out of range 1-%d\n",Kv,Nv/3);
               Kn = 0;
               Kt = 0;
            }
            //  This is an error
            else
               Fatal(22, "Invalid facet %s\n",str);
            //  Draw vectors
